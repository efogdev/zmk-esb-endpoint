...
/* Periodic beacon work */
static struct k_work_delayable beacon_work;

static void build_device_id(void)
{
    uint32_t lo = nrf_ficr_deviceid_get(NRF_FICR, 0);
    uint32_t hi = nrf_ficr_deviceid_get(NRF_FICR, 1);
...
static void beacon_work_fn(struct k_work *w)
{
    ...
    send_beacon();
    k_work_reschedule(&beacon_work, K_MSEC(BEACON_INTERVAL_MS));
}
...
void pairing_start(void)
{
    if (m_has_stored_peer) {
        LOG_INF("ESB: known dongle, going CONNECTED");
...
void pairing_on_rx(const uint8_t *data, uint8_t len)
{
    ...
    case ESB_PKT_PAIR_REQ:
        if (m_state == PAIRING_STATE_UNPAIRED) {
            LOG_INF("ESB: PAIR_REQ received");
            k_work_cancel_delayable(&beacon_work);
            m_state = PAIRING_STATE_PAIRING;
            send_pair_resp();
            save_paired();
    ...
    case ESB_PKT_DISCONNECT:
        ...
        settings_save_one(SETTINGS_KEY, (uint8_t[]){0}, 1);
        k_work_reschedule(&beacon_work, K_MSEC(10));
        break;
